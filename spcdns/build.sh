...
do
    ...
    *)
    ...
    if [[ -f $_basedir/lock ]] ; then
        [[ -f $_basedir/../_BINARY/${ARCH}/lib/lua/${LUA_VERSION}/org/conman/dns.so ]] && continue
    fi
    ...
    export CC="ccache ${CROSS_COMPILE}-gcc --sysroot=${PLATFORM}/arch-${ARCH} -g -O2 -fPIC -I$_basedir/../luajit/src/src -std=c99 -lm"
    
    rm -rf $_basedir/build
    cp -r $_basedir/src $_basedir/build
    cd $_basedir/build
    make PREFIX=/system lua || break
    
    

    rm -f $_basedir/../_BINARY/${ARCH}/lib/lua/${LUA_VERSION}/org/conman/dns.so
    mkdir -p $_basedir/../_BINARY/${ARCH}/lib/lua/${LUA_VERSION}/org/conman
    cp ./built/dns.so $_basedir/../_BINARY/${ARCH}/lib/lua/${LUA_VERSION}/org/conman/dns.so
    ${CROSS_COMPILE}-strip $_basedir/../_BINARY/${ARCH}/lib/lua/${LUA_VERSION}/org/conman/dns.so
