...
do
    ...
    *)
    ...
    if [[ -f $_basedir/lock ]] ; then
        [[ -f $_basedir/../_BINARY/${ARCH}/lib/lua/${LUA_VERSION}/compat52/bit32.so ]] && continue
    fi
    ...
    export CC="ccache ${CROSS_COMPILE}-gcc --sysroot=${PLATFORM}/arch-${ARCH} -g -O2 -fPIC -I$_basedir/../luajit/src/src -I$_basedir/src/c-api"
    
    rm -rf $_basedir/build
    cp -r $_basedir/src $_basedir/build
    cd $_basedir/build
    mkdir -p $_basedir/../_BINARY/${ARCH}/lib/lua/${LUA_VERSION}/
    $CC -c  lbitlib.c -o lbitlib.o
    $CC -shared -o $_basedir/../_BINARY/${ARCH}/lib/lua/${LUA_VERSION}/bit32.so  lbitlib.o || continue
    ${CROSS_COMPILE}-strip $_basedir/../_BINARY/${ARCH}/lib/lua/${LUA_VERSION}/bit32.so
    

    rm -rf $_basedir/../_BINARY/${ARCH}/share/lua/${LUA_VERSION}/compat52 $_basedir/../_BINARY/${ARCH}/share/lua/${LUA_VERSION}/compat52.lua
    mkdir -p $_basedir/../_BINARY/${ARCH}/share/lua/${LUA_VERSION}/compat52
    cp ./compat52/*.lua $_basedir/../_BINARY/${ARCH}/share/lua/${LUA_VERSION}/compat52
    cp ./compat52.lua $_basedir/../_BINARY/${ARCH}/share/lua/${LUA_VERSION}/
    rm $_basedir/lock
