...
do
    ...
    *)
    ...
    if [[ -f $_basedir/lock ]] ; then
        [[ -f $_basedir/../_BINARY/${ARCH}/lib/lua/${LUA_VERSION}/rex_onig.so ]] && continue
    fi
    ...
    export CC="ccache ${CROSS_COMPILE}-g++ --sysroot=${PLATFORM}/arch-${ARCH} -g -O2 -fPIC -I$_basedir/../${INCLUDE_PATH} -I$_basedir/../luajit/src/src -L$_basedir/../${LIBRARY_PATH}/${ARCH}"
    ...
    rm -f $_basedir/../_BINARY/${ARCH}/lib/lua/${LUA_VERSION}/rex_onig.so 
    mkdir -p $_basedir/../_BINARY/${ARCH}/lib/lua/${LUA_VERSION}/
    cp ./rex_onig.so $_basedir/../_BINARY/${ARCH}/lib/lua/${LUA_VERSION}/rex_onig.so
    ${CROSS_COMPILE}-strip $_basedir/../_BINARY/${ARCH}/lib/lua/${LUA_VERSION}/rex_onig.so
