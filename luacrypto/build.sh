...
do
    ...
    *)
    ...
    if [[ -f $_basedir/lock ]] ; then
        [[ -f $_basedir/../_BINARY/${ARCH}/lib/lua/${LUA_VERSION}/crypto.so ]] && continue
    fi
    ...
    export LDFLAGS="--sysroot=${PLATFORM}/arch-${ARCH} -g -O2 -fPIC"
    ...
    rm -f $_basedir/../_BINARY/${ARCH}/lib/lua/${LUA_VERSION}/crypto.so 
    mkdir -p $_basedir/../_BINARY/${ARCH}/lib/lua/${LUA_VERSION}/
    cp ./crypto.so $_basedir/../_BINARY/${ARCH}/lib/lua/${LUA_VERSION}/crypto.so
    ${CROSS_COMPILE}-strip $_basedir/../_BINARY/${ARCH}/lib/lua/${LUA_VERSION}/crypto.so
