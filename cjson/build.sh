...
do
    ...
    *)
    ...
    if [[ -f $_basedir/lock ]] ; then
        [[ -f $_basedir/../_BINARY/${ARCH}/lib/lua/${LUA_VERSION}/cjson.so ]] && continue
    fi
    ...
    export CFLAGS=" --sysroot=${PLATFORM}/arch-${ARCH} -g -O2 -fPIC -I$_basedir/../luajit/src/src -DLUA_USE_DLOPEN"
    ...
    rm -f $_basedir/../_BINARY/${ARCH}/lib/lua/${LUA_VERSION}/cjson.so 
    mkdir -p $_basedir/../_BINARY/${ARCH}/lib/lua/${LUA_VERSION}/
    cp ./cjson.so $_basedir/../_BINARY/${ARCH}/lib/lua/${LUA_VERSION}/cjson.so
    ${CROSS_COMPILE}-strip $_basedir/../_BINARY/${ARCH}/lib/lua/${LUA_VERSION}/cjson.so
