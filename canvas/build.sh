...
do
    ...
    *)
    ...
    if [[ -f $_basedir/lock ]] ; then
        [[ -f $_basedir/../_BINARY/${ARCH}/lib/lua/${LUA_VERSION}/canvas.so ]] && continue
    fi
    ...
    CMD="$CC $FLAGS src/canvas.c -c -o src/canvas.o"
    echo $CMD
    $CMD || break
    CMD="$CC $FLAGS src/compat-5.3.c -c -o src/compat-5.3.o"
    echo $CMD
    $CMD || break
    CMD="$CC $FLAGS src/canvas.o src/compat-5.3.o -shared -o $_basedir/../_BINARY/${ARCH}/lib/lua/${LUA_VERSION}/canvas.so"
    echo $CMD
    $CMD || break
    rm -f src/*.o
    ${CROSS_COMPILE}-strip $_basedir/../_BINARY/${ARCH}/lib/lua/${LUA_VERSION}/canvas.so
