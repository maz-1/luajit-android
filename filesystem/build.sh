...
do
    ...
    *)
    ...
    if [[ -f $_basedir/lock ]] ; then
        [[ -f $_basedir/../_BINARY/${ARCH}/lib/lua/${LUA_VERSION}/lfs.so ]] && continue
    fi
    ...
    CMD="$CC $FLAGS src/lfs.c -c -o src/lfs.o"
    echo $CMD
    $CMD || break
    CMD="$CC $FLAGS src/lfs.o -shared -o $_basedir/../_BINARY/${ARCH}/lib/lua/${LUA_VERSION}/lfs.so"
    echo $CMD
    $CMD || break
    rm -f src/*.o
    ${CROSS_COMPILE}-strip $_basedir/../_BINARY/${ARCH}/lib/lua/${LUA_VERSION}/lfs.so
